...
function OnError(err)
	...
	if ReplicatedStorage:FindFirstChild("BL_ARTEHACK") then
		ReplicatedStorage:FindFirstChild("BL_ARTEHACK"):Destroy()
	end
...
local succes, err = pcall(function()
	...
	if ReplicatedStorage:FindFirstChild("BL_ARTEHACK") then
		...
		local Bool = Instance.new("BoolValue", ReplicatedStorage)
		Bool.Name = "BL_ARTEHACK"
		Bool.Value = true
	end
	...
	if a == true then
		...
		if makefolder then
			...
			if isfolder then
				...
				if not isfolder("ArteHack/BL") then
					makefolder("ArteHack/BL")
				end
			end
		else
		...
		for i=1, 100 do
			...
			local suc2, err2 = pcall(function()
				...
				elseif i == 40 then
					if hasQueTelFunc then
						...
						Players.LocalPlayer.OnTeleport:Connect(function(State)
							TeleportCheck = true
							queue_on_teleport("loadstring(game:HttpGet(\"https://raw.githubusercontent.com/JustARocketGame/ArteHack/refs/heads/main/BloxFruits.lua\"))()")
						end)
					end
				...
				elseif i == 75 then
					...
					local NotifyFruit = false
					
					local FruitWait = false
					...
					task.spawn(function()
						while task.wait() do
							if isEnabled == true then
								...
								if FruitGui then
									if NotifyFruit == true and workspace:FindFirstChild("Fruit ") then
										FruitGui.Enabled = true
										if FruitWait == false then
											FruitWait = true
											SoundService:WaitForChild("FruitNotifySound_AH"):Play()
											task.spawn(function()
												wait(3)
												FruitWait = false
											end)
					...
					AddButton("Notify Fruit", true, function(enabled)
						NotifyFruit = enabled
					end)
				...
				elseif i == 90 then
					...
					if not SoundService:FindFirstChild("FruitNotifySound_AH") then
						local Sound = Instance.new("Sound", SoundService)
						Sound.Name = "FruitNotifySound_AH"
				...
				elseif i == 93 then
					...
					FruitGui = MakeGui(RandomName(10).. "_fruit", 5, true)
					FruitGui.Enabled = false

					local MainFrame = Instance.new("TextLabel", FruitGui)
					MainFrame.Position = UDim2.new(0.5, 0, 0.043, 0)
					MainFrame.AnchorPoint = Vector2.new(0.5, 0)
					MainFrame.Size = UDim2.new(0.427, 0, 0.331, 0)
					MainFrame.BackgroundColor3 = Color3.new(0.14902, 1, 0)
					MainFrame.BorderSizePixel = 0
					MainFrame.TextColor3 = Color3.new(1, 1, 1)
					MainFrame.Text = "FRUIT HAS SPAWNED!!!"
